(function () {
    ...
    function St(i) {
        ...
        return {
            c() {
                e = Wt$1("input"), ce(e, "type", "datetime-local"), ce(e, "min", i[1]), ce(e, "max", i[2]), ce(e, "class", "form-input mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 ");
            },
            m(o, r) {
        ...
        const c = () => {
            ...
            try {
                g({
                    __last_date_range: {
                        mode: r,
                        begin: s ? s.getTime() : null,
                        end: l ? l.getTime() : null
                    }
                });
            } catch (err) {}
        };
        he(() => {
            // 读取上次选择的时间区间（直接用 chrome.storage.sync，避免与局部变量 n 冲突）
            if (chrome.storage && chrome.storage.sync) {
                chrome.storage.sync.get("__last_date_range", result => {
                    const saved = result && result.__last_date_range;
                    if (saved && saved.mode) {
                        r = saved.mode, t(1, r);
                        if (saved.begin) { s = new Date(saved.begin), t(2, s); }
                        if (saved.end) { l = new Date(saved.end), t(3, l); }
                    } else {
                        ae(we()).then(_ => {
                ...
                const saved = JSON.parse(localStorage.getItem("__storage__") || "{}").__last_date_range;
                if (saved && saved.mode) {
                    r = saved.mode, t(1, r);
                    if (saved.begin) { s = new Date(saved.begin), t(2, s); }
                    if (saved.end) { l = new Date(saved.end), t(3, l); }
        ...
        constructor(e, t = !1) {
            ...
            N(this, "doDownload", async () => {
                ...
                try {
                    ...
                    // 双重保险：检查是否为MP3文件
                    if (t.name && t.name.toLowerCase().endsWith('.mp3')) {
                        console.log('队列中跳过MP3文件:', t.name);
...
`);
        ...
        addDownloadTask(e) {
            // 过滤MP3文件后再添加到队列
            const filteredFiles = e.filter(file => {
                const isMp3 = file.name && file.name.toLowerCase().endsWith('.mp3');
                if (isMp3) {
                    console.log('添加任务时过滤MP3文件:', file.name);
                }
                return !isMp3;
            });
            this.queue.push(...filteredFiles);
            // 立即更新显示队列
            pe.set(this.queue.slice(0, 8));
            // 延迟启动下载，让 UI 有时间渲染下载列表
        ...
        onError(e) {
            ...
            K.set(j.ErrorNotice), this.mode === "export" && this.exportUrl();
        }
        exportUrl() {
            var n;
        ...
        async downloadFiles(e, t) {
            this.mode = e ? "export" : "download", s.d(this.mode + " files"), await this.initSettings(), this.groupInfo = await ae(this.groupId), this.downloadFilesBetween(t);
        }
        ...
        async downloadFilesBetween(e) {
            try {
                ...
                // end_time 设为所选分钟的最后时刻，begin_time 设为所选分钟的开始时刻
                t && t.setSeconds(59, 999);
                n && n.setSeconds(0, 0);
                const beginMs = n && n.getTime(),
                    endMs = t && t.getTime();
                const o = parseInt("20");
                let r = 0,
                    s = {
                ...
                for (; !s || !s.completed;) {
                    let rawList = await Nt(this.groupId, o, s == null ? void 0 : s.create_time);
                    if (!rawList || rawList.length === 0) {
                        s = ee(A({}, s), { completed: !0 });
                        continue;
                    }
                    // 用原始列表的最后一个文件（最旧）作为下一轮游标
                    let lastFile = rawList[rawList.length - 1];
                    // 如果最旧的文件已早于 begin_time，这是最后一页
                    let isLastPage = !beginMs || new Date(lastFile.create_time).getTime() <= beginMs;
                    // 严格按时间范围 [begin_time, end_time] 过滤
                    let l = rawList.filter(f => {
                        const fileMs = new Date(f.create_time).getTime();
                        const beginOk = !beginMs || fileMs >= beginMs;
                        const endOk = !endMs || fileMs <= endMs;
                    ...
                    l = l.filter(file => {
                        const isMp3 = file.name && file.name.toLowerCase().endsWith('.mp3');
                        if (isMp3) {
                            console.log('过滤MP3文件:', file.name);
                        }
                        return !isMp3;
                    });
                    if (l.length > 0) {
                        r += l.length, this.addDownloadTask(l);
                    }
                    // 更新游标或标记完成
                    if (isLastPage || rawList.length < o || this.interrupted) {
                        s = ee(A({}, s), { completed: !0 });
                    } else {
                        s = lastFile;
                    }
                        ...
                        !I || setTimeout(() => {
                            ...
                            Z.className = "download", Z.innerText = "下载", Z.addEventListener("click", async () => {
                                ...
                                try {
                                    ...
                                    if (!xe) return;
                                    // 检查是否为MP3文件，如果是则跳过下载
if (ce.toLowerCase().endsWith('.mp3')) {
    console.log('跳过MP3文件:', ce);
    return;
}
ge({
                                        download_url: xe,
